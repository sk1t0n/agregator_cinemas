class ParserKino11Ru
  ...
  def fill_movie_names
    ...
    self.html.css('#fast_s_data>p').each { |p| self.movie_names << p.text }
    self.movie_names.delete_at(-1)
  end

  def add_movies_age_rating
    self.html.css('.kp3>div').each_with_index do |div, i| 
      @movies[self.movie_names[i]] = { :age_rating => div.text }
    end
  end

  def add_movies_image
    self.html.css('.kp>img').each_with_index do |img, i| 
      @movies[self.movie_names[i]][:image] = 'http://kino11.ru/' + img['src']
    end
  end

  def add_movies_data
    self.html.css('.slotFilm article.artH div.articleBody3 span.inf2').each_with_index do |span, i|
      if i % 6 == 0
        @movies[self.movie_names[i / 6]][:country] = span.text
      elsif i % 6 == 1
        @movies[self.movie_names[i / 6]][:producer] = span.text
      elsif i % 6 == 2
        @movies[self.movie_names[i / 6]][:genre] = span.text
      elsif i % 6 == 3
        @movies[self.movie_names[i / 6]][:budget] = span.text
      elsif i % 6 == 4
        @movies[self.movie_names[i / 6]][:duration] = span.text
      elsif i % 6 == 5
        @movies[self.movie_names[i / 6]][:actors] = span.text
      end 
    end
    html.css('.slotFilm div.inf3>p').each_with_index do |p, i|
      @movies[self.movie_names[i]][:description] = p.text
    end
  end
  ...
  def add_movies_cinemas
    self.html.css('div.slotFilm div.inf div.namef ~ div.articleBody2').each_with_index do |div, i|
      add_cinema_to_movies(div, 'div.maxi_k div.str', self.movie_names, i, 'maxi')
      add_cinema_to_movies(div, 'div.rublik_k div.str', self.movie_names, i, 'rublik')
      add_cinema_to_movies(div, 'div.mori_k div.str', self.movie_names, i, 'mori')
      add_cinema_to_movies(div, 'div.parma_k div.str', self.movie_names, i, 'parma')
      add_cinema_to_movies(div, 'div.raduga_k div.str', self.movie_names, i, 'raduga')
    end
  end

  def add_movies_ratings
    rating_kp_img = self.html.css('.kp>img')
